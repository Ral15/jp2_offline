...
describe('Delete Outcome test', function () {
  /**
  ...
  * Test Delete outcome 
  *
  * Test if the delete-outcome button exists in the
  * application.
  */
  it('should see delete outcome button with id', async function () {
    ...
    return client.setValue('#username',config.username)
      ...
      .then(() => {
        return client.$('#delete-outcome-' + transaction._id);
      })
      .then((incomeBtn) => {
  ...
  * Test delete outcome
  *
  * This test will check if a modal shows before deleting an outcome
  */
  it('should see modal before deleting an outcome', async function () {
    const client = this.app.client;
    return client.setValue('#username',config.username)
      .setValue('#password', config.password)
      .click('#submit-login')
      ...
      .click('#show-transactions-view')
      .waitForVisible('#transactions-section')
      .click('#delete-outcome-' + transaction._id)
      ...
      .then((modalText) => {
        assert.equal(modalText, 'No podras recuperar al egreso después de esta acción.');
      });
  });
/**
  * Test delete outcome
  ...
  */
  it('should cancel delete outcome sweetalert2', async function () {
    const client = this.app.client;
    return client.setValue('#username',config.username)
      .setValue('#password', config.password)
      .click('#submit-login')
...
/**
  * Test delete outcome
  *
  * This test will delete a outcome
  */
  it('should delete outcome', async function () {
    const client = this.app.client;
    return client.setValue('#username',config.username)
      .setValue('#password', config.password)
      .click('#submit-login')
      ...
      .then((totalCount) => {
        assert.isBelow(totalCount, totalOutcomes);
      })
  });  
});
